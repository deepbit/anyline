...
//加载数据 
//init:是否重新加载
function _pf_navi(init){
	_pfLoad({url:default_template_data_url,parser:default_template_data_parser,init:init});	
}
/*
		参数:
	...
	*/
	function _pfLoad(conf){
		var template_url = '/tmp/load_template_style'; 
		var template_key = conf.template || data_template_style;
		if(!template_url || !template_key){
			return;
		}
		var template_body = templates[template_key];
		if(!template_body && template_url){
			//加载样式模板
			al.ajax({
				url:template_url,
	...
	*/
	...
	function _pfRender(template_body,data, conf){
		...
		if(!container[0]){
			container = $('<div class="render_container"></div>');
